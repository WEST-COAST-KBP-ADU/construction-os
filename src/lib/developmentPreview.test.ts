...
describe("development-preview release surface (WORK-ORDER-005)", () => {
  ...
  it("keeps every configured sentence of the release truth byte-exact", () => {
    // P2-CHROME-CANON-ALIGN-0001, canon E12: calming the presentation must
    // never cost a configured sentence. Dropping, shortening, truncating, or
    // euphemizing any part of the release status must fail here.
    const html = renderToStaticMarkup(createElement(DevelopmentNotice));
  ...
  it("renders the release status as one calm line, never a plaque", () => {
    ...
    // Markup-layer probe: the label stays sentence case in configuration and
    // carries no badge, pill, or alert semantics.
    expect(siteConfig.developmentNotice.label).not.toBe(
      siteConfig.developmentNotice.label.toUpperCase(),
    );
    expect(html).not.toMatch(
      /role="alert"|aria-live|class="[^"]*(?:badge|pill|banner|alert)/i,
    );
